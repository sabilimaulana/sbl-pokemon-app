...
const ExchangeCenter: NextPage<Props> = ({ pokemons, user, myPokemons }) => {
  const { login } = useAuth();
  ...
  const myPokemonsId = myPokemons.map((pokemon) => {
    return pokemon._id;
  });
  ...
  const handleExchange = async (pokemon: Pokemon) => {
    try {
      ...
      setWarning("");
      const data = {
        firstPokemon: pokemon._id,
        secondPokemon: pokemon.wantedPokemonId,
      };
                ...
                return (
                  ...
                  >
                    <div className="p-4 rounded border-2 mx-4 transform transition duration-300 cursor-pointer hover:scale-110 hover:border-gray-700">
                      ...
                      <p>
                        Wanted pokemon
                        <span> : {pokemon.wantedPokemonName}</span>
                      </p>
                      {myPokemonsId.includes(pokemon.wantedPokemonId) && (
                        <button
                          onClick={() => handleExchange(pokemon)}
                          className="py-2 px-4 mt-4 rounded text-white bg-blue-500 hover:bg-blue-800"
                        >
                          Exchange
                        </button>
